...
@section('title', __('dashboard.pending_laundries'))

@section('content')
    <!-- Page Header -->
    <div class="page-header">
        <div class="header-content">
            <h1>{{ __('dashboard.pending_laundries') }}</h1>
            <p>{{ __('dashboard.manage_pending_laundry_requests') }}</p>
        </div>
    ...
    <div class="section-container">
                    ...
                    <tbody>
                        @foreach($laundries as $laundry)
                            <tr>
                                <td>
                                    <div class="laundry-info">
                                        <div class="laundry-name">
                                            {{ $laundry->getTranslation('name', app()->getLocale()) }}
                                        </div>
                                        @if($laundry->description)
                                            <div class="laundry-description">
                                                {{ Str::limit($laundry->getTranslation('description', app()->getLocale()), 50) }}
                                            </div>
                                        @endif
                                    </div>
                                </td>
                                ...
                                <td>
                                    @if($laundry->city)
                                        {{ $laundry->city->getTranslation('name', app()->getLocale()) }}
                                    @else
                                ...
                                <td>
                                    <div class="action-buttons">
                                        ...
                                        <form method="POST" action="{{ route('admin.laundries.approve', $laundry) }}" style="display: inline;">
                                            @csrf
                                            <button type="submit" class="action-btn approve" title="{{ __('dashboard.approve') }}" onclick="return confirm('{{ __('dashboard.are_you_sure_approve_laundry') }}')">
                                                <i class="fas fa-check"></i>
                                            </button>
                                        </form>
                                        <form method="POST" action="{{ route('admin.laundries.reject', $laundry) }}" style="display: inline;">
                                            @csrf
                                            <button type="submit" class="action-btn reject" title="{{ __('dashboard.reject') }}" onclick="return confirm('{{ __('dashboard.are_you_sure_reject_laundry') }}')">
        ...
        @else
            <div class="no-data">
                ...
                <h3>{{ __('dashboard.no_pending_laundries') }}</h3>
                <p>{{ __('dashboard.all_laundry_requests_processed') }}</p>
            </div>
        @endif
    </div>
@endsection
