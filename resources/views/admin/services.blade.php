...
@section('title', trans('dashboard.Service Management'))

@section('content')
<!-- Page Title -->
<div class="page-header">
    <h1>{{ trans('dashboard.Service Management') }}</h1>
</div>
...
<div class="search-section">
    <div class="search-container">
        ...
        <input type="text" placeholder="{{ trans('dashboard.Search services...') }}" class="search-input" id="serviceSearch">
    </div>
</div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if($service->price)
                        {{ number_format($service->price, 2) }} {{ trans('dashboard.SAR') }}
                    @elseif($service->coin_cost)
                        {{ $service->coin_cost }} {{ trans('dashboard.Coins') }}
                    @else
                ...
                <td>
                    <span class="status-badge {{ $service->status ?? 'active' }}">
                        {{ trans('dashboard.' . ucfirst($service->status ?? 'active')) }}
                    </span>
                </td>
                <td>{{ $service->created_at ? $service->created_at->format('Y-m-d') : 'N/A' }}</td>
                <td class="actions">
                    <a href="{{ route('admin.services.view', $service) }}" class="action-btn view" title="{{ trans('dashboard.View') }}">
                        <i class="fas fa-eye"></i>
                    </a>
                    <a href="{{ route('admin.services.edit', $service) }}" class="action-btn edit" title="{{ trans('dashboard.Edit') }}">
                        <i class="fas fa-edit"></i>
                    </a>
                    <button class="action-btn delete" onclick="deleteService({{ $service->id }})" title="{{ trans('dashboard.Delete') }}">
            ...
            @empty
            <tr>
                <td colspan="7" class="text-center">{{ trans('dashboard.No services found') }}</td>
            </tr>
            @endforelse
        </tbody>
    </table>
...
function deleteService(serviceId) {
    if (confirm('{{ trans("dashboard.Are you sure you want to delete this service?") }}')) {
